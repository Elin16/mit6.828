...
mem_init(void)
{
	...
	uint32_t env_mem_size = sizeof(struct Env) * NENV;
	envs = (struct Env *) boot_alloc(env_mem_size);
	memset(envs, 0, env_mem_size);
	//////////////////////////////////////////////////////////////////////
	...
	//cprintf("We are checking !");
	check_kern_pgdir();
	//cprintf("We are finished !");
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if( (pde & PTE_P ) == 0){// The relevant page table page not exist.
		if(create){
			...
			pgdir[pde_index] = new_page_pa | PTE_P | PTE_U | PTE_W;
			pde = pgdir[pde_index];
		}else{
			return NULL;
		}
	...
	return (pte_t*) KADDR(PTE_ADDR(pde)) + pte_index;
}
...
{
	// LAB 3: Your code here.
	uint32_t lva = (uint32_t)ROUNDDOWN(va, PGSIZE), rva = (uint32_t)ROUNDUP(va + len, PGSIZE);
    int check_perm = (perm | PTE_P);
    uint32_t check_va = (uint32_t)va;
    for (; lva < rva; lva += PGSIZE) {
        pte_t *pte = pgdir_walk(env->env_pgdir, (void *)lva, 0);
        if ((lva >= ULIM) || !pte || (*pte & check_perm) != check_perm) {
			if(lva >= check_va){
